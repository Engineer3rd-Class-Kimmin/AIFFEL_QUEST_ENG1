...
### 1. 데이터 정제 또는 학습 전략을 활용한 정량적 성능 향상

- [x] 기존 KoChatGPT 데이터에서 완성문 앞의 불필요한 따옴표, 다중 공백, 짧은 샘플과 중복 프롬프트를 정제했습니다.
- [x] 정제 대상 원본 1,500개를 1,491개로 정리한 뒤 정중한 요청 접두사 증강을 적용하여 2,239개로 확장했습니다.
- [x] Greedy, Beam Search, Top-k·Top-p Sampling을 비교하고 `no_repeat_ngram_size=2`를 사용해 반복 생성을 억제했습니다.
- [x] 기존 KoGPT-2에 LoRA 기반 SFT를 적용하여 모든 정량 지표가 향상됐습니다.

| 모델 | BLEU | ROUGE-L | ROUGE-1 |
|---|---:|---:|---:|
| Baseline KoGPT-2 | 0.0220 | 0.1122 | 0.2055 |
| SFT-LoRA | 0.0865 | 0.1911 | 0.3346 |

> 정량 지표는 전체 정제·증강 및 SFT 파이프라인을 적용한 결과입니다. 정제 전·후 모델을 동일 조건으로 비교한 ablation 실험은 없으므로, 향상 폭을 데이터 정제만의 효과로 단정하지 않고 SFT를 포함한 전체 학습 전략의 효과로 해석하는 것이 적절합니다. 디코딩 전략은 동일 프롬프트의 생성 결과를 통해 정성적으로 비교했습니다.

### 2. SFT 모델과 RM 적용 결과 분석

- [x] Reward Model은 370개의 chosen/rejected 쌍으로 학습했으며 최종 pairwise accuracy `0.959`를 기록했습니다.
- [x] sanity check에서 chosen 응답의 보상은 `-1.667`, rejected 응답의 보상은 `-3.520`으로 나타나 사람의 선호 순서를 학습했음을 확인했습니다.
- [x] SFT 모델과 RM 보상으로 최적화한 PPO 정책 모델의 응답을 동일 프롬프트에서 생성하고, 각 응답에 대한 RM 보상 점수를 정량·정성적으로 비교했습니다.
- [x] PPO rollout의 평균 보상은 `[0.294, 0.142, -0.715, -2.024]`로 안정적으로 상승하지 않았습니다. 이를 모델 개선으로 과장하지 않고, 4회의 짧은 rollout과 작은 batch를 사용한 CPU 축소 실험의 한계로 분석했습니다.

> RM은 문장을 생성하는 모델이 아니라 응답에 선호 점수를 부여하는 모델입니다. 따라서 본 노트북에서는 “SFT 모델과 RM 모델의 생성 결과”가 아니라, **SFT 모델과 RM 보상으로 최적화한 PPO 정책 모델의 생성 결과**를 비교했습니다.

### 3. 기존 KoGPT-2와 SFT 적용 모델 비교

- [x] 학습에 사용하지 않은 held-out 샘플 20개에서 기존 KoGPT-2와 SFT-LoRA 모델의 BLEU·ROUGE를 비교했습니다.
- [x] 한국어 평가에 적합하도록 공백을 제거한 글자 단위 토큰화를 적용했습니다.
- [x] `세계에서 가장 높은 산은 무엇인가요?`, `건강하게 살을 빼는 방법 알려줘`, `파이썬으로 파일을 읽는 방법은?`이라는 동일한 질문에 대한 Baseline·SFT·PPO 응답과 RM 보상 점수를 함께 제시해 정성적으로 비교했습니다.
- [x] SFT-LoRA가 모든 정량 지표에서 Baseline보다 높은 점수를 기록했지만, 평가 샘플이 20개이고 학습 설정이 300개 샘플·1 epoch로 축소되어 있으므로 절대적인 성능보다 상대적인 향상 경향으로 해석했습니다.
...
KoChatGPT의 SFT·RM·PPO 파이프라인을 단순히 재현하는 데 그치지 않고, 데이터 포맷 오류를 수정하고 정제·증강, LoRA, 디코딩 비교, 한국어 평가 방식까지 확장한 점이 좋았습니다. 기존 KoGPT-2와 SFT-LoRA의 정량·정성 비교는 평가 기준을 명확히 충족하며, RM의 선호 학습 결과도 pairwise accuracy와 보상 점수로 확인했습니다. 다만 데이터 정제와 디코딩 전략 각각의 독립적인 효과를 확인하는 ablation 실험, 그리고 여러 평가 프롬프트에서 계산한 SFT와 PPO의 평균 RM 보상 비교가 추가되면 평가 근거가 더욱 탄탄해질 것입니다. 특히 모든 단계를 CPU에서 실행할 수 있도록 구성하면서도 성공한 결과와 실패한 결과를 모두 수치로 남겨 재현성과 이해도를 높인 점이 인상적입니다.
